...
public class Vector {
  ...
  public static Vector fromReals(double... cs) {
    return new Vector(
        Arrays.stream(cs).mapToObj(ComplexNumber::real).toArray(ComplexNumber[]::new));
  }
  ...
  public static ComplexNumber innerProduct(Vector v1, Vector v2) {
    ...
    for (int i = 0; i < v1.size(); i++) {
      dotProd =
          ComplexNumber.sum(
              dotProd,
              ComplexNumber.product(v1.components.get(i).conjugate(), v2.components.get(i)));
    }
  ...
  public ComplexNumber getComponent(int i) {
    if (i < 1 || i > components.size())
      throw new IllegalArgumentException("Accessing out of bound component");
