...
public class Matrix {
  ...
  private Matrix(Vector... columns) {
    boolean columnVectorsAllSameDimension =
        Arrays.stream(columns).map(Vector::size).distinct().count() == 1;
    if (!columnVectorsAllSameDimension)
      throw new IllegalArgumentException("Column vectors must all be of same size.");
  ...
  /**
   * Creates a matrix with real elements of a specific size, with entries inputted going across. So
   * the first n doubles inputted correspond to the first row of the matrix.
   *
  ...
  public String toString() {
    return "LA.Matrix{" + "rows=" + Arrays.toString(getRows()) + '}';
  }
